		...
		<c:if test="<%= ListUtil.isNotNull(headerNames) %>">
			<thead class="table-columns">
				...
				for (int i = 0; i < headerNames.size(); i++) {
					...
					String sortedMessageKey = StringPool.BLANK;
					...
					if (Objects.equals(orderByType, "asc")) {
						orderByType = "desc";
						sortedMessageKey = "ascending";
					}
					else {
						orderByType = "asc";
						sortedMessageKey = "descending";
					}
				%>
					...
					<th
						...
						<c:if test="<%= (orderKey != null) && orderCurrentHeader %>">
							aria-live="assertive" aria-sort="<%= LanguageUtil.get(resourceBundle, sortedMessageKey) %>"
						</c:if>
					>
						<c:if test="<%= orderKey != null %>">
							<div class="table-sort-liner">
								...
								if (orderCurrentHeader) {
									sortLinkTitle = LanguageUtil.get(resourceBundle, sortedMessageKey);
								}
								%>
